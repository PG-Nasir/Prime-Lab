...
List<OrganizationInfo> orgranizationInfo = (List<OrganizationInfo>) session.getAttribute("organization_info");
    ...
    try {
		...
		String Sql="";
		if(testId.equals("0")){
			Sql="select *,(select organizationLogo from tbOrganizationInfo where organizationId='1' ) as OrgLogo,'Test Wise Investigation Statement' as ReportTitle,(select PatientName from TbLabPatient where labId=tblabtesthistory.labId and FiscalYear=tblabtesthistory.FiscalYear and CMonth=tblabtesthistory.CMonth) as PatientName,(select SampleCollect from TbLabPatient where labId=tblabtesthistory.labId and FiscalYear=tblabtesthistory.FiscalYear and CMonth=tblabtesthistory.CMonth) as SampleCollect,'"+fromDate+"' as StartDate,'"+toDate+"' as enddate,1 as Qty,(select (select GroupName from tblabtestgroup where HeadId=tbTestName.HeadId)  from tbTestName where TestName=tblabtesthistory.testName) as GroupHead  from tblabtesthistory where type=1 and  RefundStatus='0' and date between '"+fromDate+"' and '"+toDate+"' and labId IS NOT NULL order by GroupHead,testName";

		}
		else{
			Sql="select *,(select organizationLogo from tbOrganizationInfo where organizationId='1' ) as OrgLogo,'Test Wise Investigation Statement' as ReportTitle,(select PatientName from TbLabPatient where labId=tblabtesthistory.labId and FiscalYear=tblabtesthistory.FiscalYear and CMonth=tblabtesthistory.CMonth) as PatientName,(select SampleCollect from TbLabPatient where labId=tblabtesthistory.labId and FiscalYear=tblabtesthistory.FiscalYear and CMonth=tblabtesthistory.CMonth) as SampleCollect,'"+fromDate+"' as StartDate,'"+toDate+"' as enddate,1 as Qty,(select (select GroupName from tblabtestgroup where HeadId=tbTestName.HeadId)  from tbTestName where TestName=tblabtesthistory.testName) as GroupHead  from tblabtesthistory where type=1 and  RefundStatus='0' and date between '"+fromDate+"' and '"+toDate+"' and testId='"+testId+"' and labId IS NOT NULL order by GroupHead,testName";
		...
		String jrxmlFile = session.getServletContext().getRealPath("WEB-INF/jasper/lab/TestWiseAllInvestigationStatement.jrxml");
