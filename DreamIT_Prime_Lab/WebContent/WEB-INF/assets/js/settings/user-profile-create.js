...
function saveAction(){
	...
	for (let i = 1; i <=rowList; i++) {
		...
		if($("#check_permit_"+rId).is(":checked")){
			...
			let moduleId=0,headId=0,subId=0,add=0,edit=0,view=0,del=0,refund,discount;
...
function drawRolePermissionTable(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        rows += `<tr data-id=${permission.subMenuId} ">
				...
				<td class="row-index text-center"> <input type="checkbox" ${permission.refund == '0' ? '' : 'checked'}> </td>
				<td class="row-index text-center"> <input type="checkbox" ${permission.discount == '0' ? '' : 'checked'}> </td>
		</tr>`;
    }
