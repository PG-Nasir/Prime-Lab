...
public class PasswordDAOImpl implements PasswordDAO{
	...
	public JSONArray getRolePermissions(String roleIds) {
		...
		try{
			...
			String sql="select p.moduleid,m.name as ModuleName,p.sub,sm.name as subMenuName,p.entry,p.edit,p.[view],p.clear,p.refund,p.discount from tbRolePermission p\n" + 
					"left join tbsubMenu sm\n" + 
					"on p.sub = sm.id\n" + 
					"left join TbModule m\n" + 
					"on p.moduleid = m.id\n" + 
					"where p.roleId in ("+roleIds+")\n" + 
					"group by  p.moduleid,m.name,p.sub,sm.name,p.entry,p.edit,p.[view],p.clear,p.refund,p.discount,sm.ordering\n" + 
			...
			{	
				...
				permissionObject.put("refund", element[8].toString());
				permissionObject.put("discount", element[9].toString());
	...
	public String saveUserProfile(String userInfo) {
		...
		try{
			...
			String sql = "insert into Tblogin (fullname,username,password,employeeId,type,factoryId,departmentId,active,createby,entrytime) values('"+permissionObject.get("fullName")+"','"+permissionObject.get("userName")+"','"+permissionObject.get("password")+"','"+permissionObject.get("employeeId")+"','1','1','1','"+permissionObject.get("activeStatus")+"','"+permissionObject.get("userId")+"',CURRENT_TIMESTAMP);";
			session.createSQLQuery(sql).executeUpdate();
			...
			for(int i=0;i<extraPermissionList.size();i++) {
				...
				sql="insert into tbUserExtraPermission (userId, moduleid, head, sub, entry, edit, [view], clear,refund,discount, entryby) values ('"+maxUserId+"','"+extraPermission.get("moduleId")+"','"+extraPermission.get("headId")+"','"+extraPermission.get("subId")+"','"+extraPermission.get("add")+"','"+extraPermission.get("edit")+"','"+extraPermission.get("view")+"','"+extraPermission.get("delete")+"','"+extraPermission.get("refund")+"','"+extraPermission.get("discount")+",'"+permissionObject.get("userId")+"')";
				session.createSQLQuery(sql).executeUpdate();
			}
	...
	public String editUserProfile(String userInfo) {
		...
		try{
			...
			for(int i=0;i<extraPermissionList.size();i++) {
				...
				sql="insert into tbUserExtraPermission (userId, moduleid, head, sub, entry, edit, [view], clear,refund,discount, entryby) values ('"+permissionObject.get("userAutoId")+"','"+extraPermission.get("moduleId")+"','"+extraPermission.get("headId")+"','"+extraPermission.get("subId")+"','"+extraPermission.get("add")+"','"+extraPermission.get("edit")+"','"+extraPermission.get("view")+"','"+extraPermission.get("delete")+"','"+extraPermission.get("refund")+"','"+extraPermission.get("discount")+"','"+permissionObject.get("userId")+"')";
				session.createSQLQuery(sql).executeUpdate();
			}
