...
/* 		System.out.println("testIdlist "+testIdlist);
		...
		while(resulttoken.hasMoreTokens()){
			...
			Sql="select TestName from TbTestName where TestId='"+testId+"'";
			
			List<?> tlist = dbsession.createSQLQuery(Sql).list();
			
			for(Iterator<?> iter = tlist.iterator(); iter.hasNext();)
			{
				testNameList=testNameList+iter.next().toString()+",";
				break;
			}
			...
			List<?> checklist = dbsession.createSQLQuery(Sql).list();
			...
			if(sub==1){
				Sql="select a.SubTestId,a.SubTestName,a.Unit,a.NormalRange,isnull((select value from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.SubTestId),'') as Result,isnull((select CalculateValue from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.SubTestId),'') as CalculateValue,isnull((select Flag from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.SubTestId),'') as Flag,(select TestName from TbTestName where TestId=a.TestId) as MainTestName,isnull((select Sorting from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.SubTestId),'') as Sorting from TbSubTestName a where a.TestId='"+testId+"'";
			}
			else{
				Sql="select a.TestId,a.TestName,a.Unit,a.NormalRange,isnull((select value from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.TestId),'') as Result,isnull((select CalculateValue from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.TestId),'') as CalculateValue,isnull((select Flag from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.TestId),'') as Flag,(select TestName from TbTestName where TestId=a.TestId) as MainTestName,isnull((select Sorting from tblabreportvalue where FiscalYear='"+fiscalYear+"' and LabId='"+labId+"' and testId='"+testId+"' and HeadId='"+headId+"'  and rId=a.TestId),'') as Sorting from TbTestName a where a.TestId='"+testId+"'";
			}
			...
			for(Iterator<?> iter = list.iterator(); iter.hasNext();)
			{
				...
				map.put("LabNo",fiscalYear+"-"+labId);
				map.put("PatientName",Name);
				Age=!Age.equals("")?Age+"Y":"";
				Month=!Month.equals("")?Month+"M":"";
				Day=!Day.equals("")?Day+"D":"";
		...
		jrxmlFile=session.getServletContext().getRealPath("WEB-INF/jasper/LabResultReport/HormoneWithOutGroup.jrxml");
