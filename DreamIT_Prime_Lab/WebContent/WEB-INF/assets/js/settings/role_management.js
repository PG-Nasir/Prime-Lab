...
function saveAction(){
	...
	for (var i = 1; i <=rowList; i++) {
		...
		if($("#check_"+rId).is(":checked")){
			...
			let module=0,head=0,sub=0,add=0,edit=0,view=0,del=0,refund=0,discount=0;
			...
			if($("#refund_"+rId).is(":checked")){
				refund=1;
			}else{
				refund=0;
			}
			
			if($("#discount_"+rId).is(":checked")){
				discount=1;
			}else{
				discount=0;
			}

			var value=module+":"+head+":"+sub+":"+add+":"+edit+":"+view+":"+del+":"+refund+":"+discount;
			accessList[j++] = [value];
		}
	}
...
function setTableData(data){
	for (var i = 0; i < data.length; i++) {
		$('#roleList').append(`<tr data-id=${data[i].subId} id="R${++rowIdx}">
				...
				<td class="row-index text-center"> <input data-sub="${data[i].subId}" id="refund_${data[i].subId}" class="refund" type="checkbox"> </td>
				<td class="row-index text-center"> <input data-sub="${data[i].subId}" id="discount_${data[i].subId}" class="discount" type="checkbox"> </td>
	...
	for (var i = 0; i < data.length; i++) {
		for (var j = 1; j <= rowL; j++) {
			...
			if(data[i].subId== rId){
				...
				if(data[i].refund == 1){
					$("#refund_"+data[i].subId).prop('checked', true);
				}else{
					$("#refund_"+data[i].subId).prop('checked', false);
				}
				
				if(data[i].discount == 1){
					$("#discount_"+data[i].subId).prop('checked', true);
				}else{
					$("#discount_"+data[i].subId).prop('checked', false);
				}
...
$("#checkAllRefund").click(function () {
	if ($(this).is(":checked")) {
		$(this).closest('table').find('td input[class="refund"]').prop('checked', this.checked);
	}else{
		$(this).closest('table').find('td input[class="refund"]').prop('checked', false);
	}
});

$("#checkAllDiscount").click(function () {
	if ($(this).is(":checked")) {
		$(this).closest('table').find('td input[class="discount"]').prop('checked', this.checked);
	}else{
		$(this).closest('table').find('td input[class="discount"]').prop('checked', false);
	}
});
