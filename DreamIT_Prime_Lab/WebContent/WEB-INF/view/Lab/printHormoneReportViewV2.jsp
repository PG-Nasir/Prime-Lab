...
String titleId = (String)request.getAttribute("titleId"); 
    ...
    try {
		...
		String CutOfValueV="";
		String PatientSampleCountV="";
		String ImpressionV="";
		
		String CutOfValueN="";
		String PatientSampleCountN="";
		String ImpressionN="";
		...
		datalist.clear();
		String Sql = "select RegNo,PatientName,Age,Month,Day,Sex,(select name+'. '+Degree+'' from TbDoctorInfo where DoctorId=TbLabPatient.RefferBy) as RefferDegree,convert(varchar,DateTime,105) as DateTime,(select TestName from tbTestName where TestId='"+testIdlist+"') as TestName from TbLabPatient where FiscalYear='"+fiscalYear+"' and cMonth='"+cMonth+"' and labId='"+labId+"'";
		...
		Sql="select tblabreportvalue.value,isnull(tblabreporthead.Ranges,0),tblabreporthead.Name,tblabreporthead.Catagory,(select name from tblogin where id=tblabreportvalue.createBy) as FullName,'' as DegreeName,(select username from tblogin where id=tblabreportvalue.createBy) as username from tblabreporthead join tblabreportvalue on tblabreportvalue.rId=tblabreporthead.Id where tblabreportvalue.FiscalYear='"+fiscalYear+"' and tblabreportvalue.LabId='"+labId+"' and tblabreportvalue.cMonth='"+cMonth+"' and tblabreportvalue.HeadId='"+headId+"' and testId='"+testIdlist+"' order by Catagory,rId asc";
		...
		for(Iterator<?> iter = list.iterator(); iter.hasNext();)
		{
			...
			if(element[2].toString().equals("Cut Of Value")){
				CutOfValueV=element[0].toString();
				CutOfValueN=element[2].toString();
			}
			if(element[2].toString().equals("Patient Sample Count")){
				PatientSampleCountV=element[0].toString();
				PatientSampleCountN=element[2].toString();
			}
			if(element[2].toString().equals("Impression")){
				ImpressionV=element[0].toString();
				ImpressionN=element[2].toString();
			}
		...
		map.put("CutOfValueV",CutOfValueV);
		map.put("CutOfValueN",CutOfValueN);
		map.put("PatientSampleCountV",PatientSampleCountV);
		map.put("PatientSampleCountN",PatientSampleCountN);
		map.put("ImpressionV",ImpressionV);
		map.put("ImpressionN",ImpressionN);
		...
		String jrxmlFile=session.getServletContext().getRealPath("WEB-INF/jasper/LabResultReport/HormoneConfirmatoryReport.jrxml");
